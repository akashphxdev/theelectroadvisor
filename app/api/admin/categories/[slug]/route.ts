...
export async function PUT(
  ...
  try {
    ...
    const slug = formData.get("slug") as string;
    const is_active = formData.get("is_active") ?? "1";
    const newImagePath = formData.get("image_path") as string | null;
    const removeImage = formData.get("remove_image");
    ...
    const isProtected = PROTECTED_SLUGS.includes(currentSlug);

    if (isProtected) {
      // Fetch original name & slug
      const [existingRows]: any = await db.query(
        "SELECT name, slug FROM tblcategories WHERE slug=?",
        [currentSlug]
      );
      ...
      // Name ya slug change karne ki koshish ki toh block
      if (name !== original.name || slug !== original.slug) {
        return NextResponse.json(
          {
            success: false,
            message: "Name and slug cannot be changed for this category",
          },
          { status: 403 }
        );
      }
    ...
    // Slug conflict check (only for non-protected)
    if (!isProtected && slug !== currentSlug) {
      const [existing]: any = await db.query(
        "SELECT id FROM tblcategories WHERE slug=?",
        [slug]
      );
    ...
    // Koi image change nahi - image_url waise hi rahega
...
export async function DELETE(
  ...
  try {
    ...
    // 🔒 Protected categories delete nahi hongi
    if (PROTECTED_SLUGS.includes(slug)) {
      return NextResponse.json(
        { success: false, message: "This category cannot be deleted" },
        { status: 403 }
      );
    }
