...
// ✅ Fix 1: Category type define kiya — 'any' hata diya
...
// ✅ Fix 3: Message type define kiya
...
export default function EditCategoryPage({ params }: Props) {
  ...
  // ✅ Fix 4: All states properly typed
  const [form, setForm] = useState<CategoryForm>({ name: "", slug: "", is_active: 1 });
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        ...
        if (data.success) {
          // ✅ Fix 5: 'any' hata ke CategoryData type use kiya
          const cat = (data.data as CategoryData[]).find((c) => c.slug === slug);
          if (cat) {
