...
const categoryColors = ["#3b82f6", "#8b5cf6", "#6366f1", "#ef4444", "#10b981", "#ec4899", "#f97316", "#f59e0b"];
...
const quickLinks = [
  ...
  {
    label: "Add Author", href: "/admin/authors",
    color: "#10b981",
...
function StatCard({ label, value, change, up, icon, color, bg, loading }) {
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs mt-1 flex items-center gap-1" style={{ color: up ? "#16a34a" : "#9ca3af" }}>
...
export default function DashboardPage() {
  ...
  const stats = data ? [
    {
      label: "Total Articles", value: String(data.stats.totalArticles), change: "All time", up: true,
      color: "#3b82f6", bg: "#eff6ff",
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>,
    },
    {
      label: "Categories", value: String(data.stats.totalCategories), change: "All time", up: true,
      color: "#8b5cf6", bg: "#f5f3ff",
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>,
    },
    {
      label: "Authors", value: String(data.stats.totalAuthors), change: "All time", up: true,
      color: "#10b981", bg: "#ecfdf5",
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>,
    },
    {
      label: "Total Views", value: String(data.stats.totalViews), change: "All articles", up: true,
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <a href="/admin/articles" className="text-xs font-semibold text-blue-600 hover:text-blue-700">View all →</a>
          </div>
                    ...
                    return (
                      <tr key={a.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3.5">
                          <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full whitespace-nowrap">{a.category}</span>
                        </td>
                        <td className="px-4 py-3.5">
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${s.bg} ${s.text}`}>{a.status}</span>
                        </td>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              {quickLinks.map((q) => (
                ...
                >
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white" style={{ backgroundColor: q.color }}>
                    {q.icon}
                  </div>
                  <span className="text-xs font-semibold text-gray-600 group-hover:text-gray-900 leading-tight">{q.label}</span>
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <a href="/admin/categories/create" className="text-xs font-semibold text-blue-600 hover:text-blue-700">View all →</a>
            </div>
            {loading ? (
                  ...
                  <div key={c.name}>
                    ...
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${c.pct}%`, backgroundColor: categoryColors[i % categoryColors.length] }}
                      />
                    </div>
                  </div>
                ))}
